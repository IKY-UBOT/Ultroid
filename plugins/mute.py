...
async def _(e):
    ...
    try:
        ...
        await eod(
            xx,
            f"`Done Unmuted Tod...` [{name}](tg://user?id={userid}) `Di {chat.title}`",
            time=5,
        )
    except BaseException as m:
        await xx.eor(f"`{m}`", time=5)
...
async def _(e):
    ...
    if e.reply_to_msg_id:
        ...
        return await xx.eor(get_string("tban_1"), time=3)
    if userid == ultroid_bot.uid:
        return await xx.eor("`Hoammm.....`", time=3)
    try:
        await e.client.edit_permissions(
            chat.id,
            userid,
        ...
        await eod(
            xx,
            f"`Udah Ya tod...` {name} `Di {chat.title}`",
        )
    except BaseException as m:
        await xx.eor(f"`{m}`", time=5)
